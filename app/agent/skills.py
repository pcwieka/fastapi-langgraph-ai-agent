...
class AgentSkills:
    ...
    def search_products(self, state: AgentState) -> dict:
        ...
        results: list[dict[str, object]] = self._product_service.search(query)
    ...
    async def generate_qa_answer(self, state: AgentState) -> dict:
        ...
        answer: str = await self._qa_generator.generate(user_message, products)
    ...
    async def prepare_order(self, state: AgentState) -> dict:
        ...
        """
        last_message: str = state["messages"][-1]["content"]
        all_products: list[dict[str, object]] = self._product_service.get_all()

        draft_result = await self._order_generator.generate(
    ...
    def finalize_order(self, state: AgentState) -> dict:
        ...
        if confirmed:
            order_id = self._order_service.create_order(state["session_id"], order)
            answer = (
    ...
    def track_order(self, state: AgentState) -> dict:
        """Look up orders for the current session and return status."""
        orders = self._order_service.find_orders(state["session_id"])
